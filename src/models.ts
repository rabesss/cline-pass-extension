...
for (const issue of runtimeCatalog.issues) console.warn(`[cline-pass] skipped catalog entry: ${issue}`);
...
export function resolveRuntimeModel(model: RuntimeModel | undefined): RuntimeModel {
  ...
  const thinkingLevelMap =
    host.thinkingLevelMap !== undefined ? structuredClone(host.thinkingLevelMap) : restored.thinkingLevelMap;
  const pricingTiers =
    host.pricingTiers !== undefined ? structuredClone(host.pricingTiers) : restored.pricingTiers;
