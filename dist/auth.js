...
import { CLINE_ACCOUNT_PROVIDER_ID, CLINE_API_BASE, CLINE_API_KEY_ENV_VAR, CLINE_PASS_ACCESS_TOKEN_ENV_VAR, CLINE_PASS_API_KEY_ENV_VAR, CLINE_PASS_OMP_AGENT_DB_ENV_VAR, CLINE_WORKOS_ACCESS_TOKEN_PREFIX, CLINE_WORKOS_API_BASE, CLINE_WORKOS_CLIENT_ID, DEFAULT_SOURCE_PATH, PROVIDER_ID, TEN_YEARS_MS, } from "./constants.js";
import { delay, expandHome, expiryTimeMs, isExpired, normalizeBaseUrl, optionalNonNegativeInteger, positiveInteger, safeError, sanitizeErrorDetail, stringValue, throwIfAborted, verificationUriWithCode, withAbortSignal, } from "./utils.js";
...
export async function readClinePassCredentials(options = {}) {
    ...
    const refreshFetchImpl = options.fetchImpl ?? globalThis.fetch;
    const refreshOptions = {
        baseUrl: options.baseUrl || env.CLINE_PASS_API_BASE || CLINE_API_BASE,
    };
    if (refreshFetchImpl)
        refreshOptions.fetchImpl = refreshFetchImpl;
...
export async function resolveRuntimeApiKey(options = {}, env = process.env) {
    ...
    if (stored) {
        ...
        const refreshOptions = {};
        if (options.baseUrl)
            refreshOptions.baseUrl = options.baseUrl;
        if (options.fetchImpl)
