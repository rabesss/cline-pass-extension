...
export function clineHTTPErrorMessage(status, body) {
    if (status === 401)
        return "Cline API returned HTTP 401. Run /login again, refresh an imported Cline app session, or set CLINE_PASS_API_KEY.";
...
export function unwrapClineResponsePayload(payload) {
...
export function nonSseStreamError(rawBody) {
    try {
        ...
        if (error instanceof SyntaxError)
            return new Error("Cline API returned a non-SSE streaming response.");
        return error instanceof Error ? error : new Error(String(error));
    }
}
function clineEnvelopeErrorDetail(payload) {
