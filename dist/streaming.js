...
function requestMaxTokens(model, options) {
...
function messagesToOpenAI(context = {}, model = {}) {
    const messages = [];
...
function userContentToOpenAI(content, supportsImages) {
...
function applyUsage(usage, source, model = {}) {
    ...
    usage.cacheRead = Math.min(promptTokens, Math.max(0, numberValue(promptDetails.cached_tokens ?? source?.cache_read_input_tokens)));
    usage.cacheWrite = Math.min(Math.max(0, promptTokens - usage.cacheRead), Math.max(0, numberValue(promptDetails.cache_write_tokens ??
